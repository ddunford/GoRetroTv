...
### And the whole chain, traced to a state number

The grid's exit was followed all the way rather than reasoned about, by tracing every instruction
the box fetched from the moment it entered `0x800ADD08` until control came back — 538 of them, read
against the listing.

**The resolver SUCCEEDS.** Handle `0x3001` — pool 3, element 0 — is not zero and is not out of
bounds. It resolves, fills the caller's buffer at `+4` and `+6`, and tags `buffer[0] = 11` with the
object's kind. Everything that looked like a candidate failure is not happening.

The 2 comes from one call the successful arm makes:

    800ade3a  lw   v1,0x800adfc4     = 0x800AC535
    800ade3c  jalr v1                call 0x800AC534
    800ade3e  move a0,s0             a0 is the object's INTERNAL STATE
    800ade40  sw   v0,16(sp)         and its answer is what the grid sees

`0x800AC534` is a pure jump table at `0x800AC548`, seven sixteen-bit offsets, and it is nothing but
a mapping from an internal state to the code a caller is allowed to see:

| internal state | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|---|---|---|---|---|---|---|---|
| code returned | 1 | 0 | **2** | **2** | 3 | 4 | a constant |

And the grid draws a row only when that code is **three or more**:

    800a4bb2  cmpi  v1,2
    800a4bb4  bteqz 0x800a4d8d        == 2 -> leave
    800a4bb8  slti  v1,2
    800a4bba  btnez 0x800a4dad        <  2 -> leave
    800a4bbe  li    v1,7              otherwise draw

**Measured off the running box: the state is 4.** So the object the grid depends on is two states
short of the first one that draws anything, and the entire screen follows from that one number.

Every link is measured rather than inferred: the loop's limit is six, the handle is `0x3001`, the
resolver returns kind 11, the state is 4, the code is 2, and the reads at `0x800A4D92`/`0x800A4D98`
that run only on the `== 2` path both executed once.

**What the state MEANS is not established and must not be guessed.** What is established is that it
is not per-channel — every one of the six channel records carries the same `entry+20 = 6`, so all
...
### The row body is not bailing out, and waiting is not the answer either

With the transport ready the loop runs six times, and each iteration does **real work**: the first
alone fetches more than four thousand instructions, through the same subsystem at
`0x800D0F00`..`0x800D3BC0` that the handle resolver walks, and out into `0x800AC400`..`0x800AF100`
by way of `0x800AC46C`, `0x800AC88C`, `0x800ACA74`, `0x800AEFCC`, `0x800AF0F4` and `0x800ADCD0`.
That is not a loop giving up. That is a loop building six rows.

**And the screen is byte for byte `42DBD889` — identical to the run where the loop executed once.**

Two readings survived that, and they wanted different work: the rows are computed and draw nothing,
or they are computed and drawn LATER than anybody has looked. Every measurement of this screen on
this project has stopped at a settle — four samples 65,536 instructions apart with no change — and a
settle is a statement about the last quarter of a million instructions, not about the screen. That
is exactly the mistake the transport state turned out to be, wearing different clothes.

So the box was kept running for **two hundred million instructions after the grid opened**, sampled
every million. **The screen never changed once.** The transport went 6 → 7 while watching and the
grid stayed empty throughout.

> The rows are computed and produce no pixels. Waiting is not the answer, and the second refusal is
> inside the body rather than after it.

That is a clean negative and it narrows the search sharply: six iterations of four thousand
