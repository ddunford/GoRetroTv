...
**IT IS ACCEPTANCE, AND THE BOX KEEPS WHAT WE SEND.** Same day, `TestWhetherTheBoxKeepsTheTableC1
Payload` replaces the zero payload with an eight-byte marker nothing else in the machine contains
and then searches all of DRAM for it. The control run, identical but for the push, does not contain
it -- checked, because a marker that is not distinctive would make this instrument lie. The pushed
run contains it **twice, the first at guest `0x801AF5D0`**, which is ordinary application heap and
not the section ring at `0x007A0000`. So the box did not merely glance at a header: it ALLOCATED,
PARSED AND COPIED OUR PAYLOAD INTO ITS OWN MEMORY.

The 544 addresses corroborate that from the other side. Clustered into 33 contiguous runs (the
2-byte stride says MIPS16 throughout) they fall in five regions, and one is already named in this
file's own listings ladder:

    0x800CCECC..0x800CCEEC   17 PCs     THE ALLOCATOR -- "findBlock 0x800CCECC alloc" above
    0x8007F0FC..0x8007F200   80 PCs     beside alloc2 at 0x8007F77C: allocation machinery
    0x800A8176..0x800A8726  186 PCs     the hottest by far -- 864 executions at 0x800A8176 alone
    0x800C4C34..0x800C514A  283 PCs     the largest region, and the 0xC1 consumer proper
    0x80004760, 0x800D1B40    8 PCs

**None of the listings ladder appears** -- no `findBlock`, no `registerBlock`, no `0x800C587C`
per-event register, no `0x800C579C` notify. This is a different consumer reached by a different
path, which is what a different table id should look like. Full census:
`.artifacts/table-c1-exclusive-pcs.txt`, regenerated by the test.

**What this still does NOT establish** is the payload FORMAT, what the stored data is for, or
whether `0x52`/extension `0x0100` are the right addressing -- they are only ones that worked. The
format stays unguessed on purpose. Either answer is progress, because the box has named a table it wants and this is
