...
func (r *Runtime) taskCount() (int, error) {
	...
	for at := seed; ; {
		if at < memory.DRAMBase || at-memory.DRAMBase+0x70 > r.RAM.Size() || at&3 != 0 {
			return 0, fmt.Errorf("task census: created list left DRAM at %08X", at)
		}
		if seen[at] {
			return 0, fmt.Errorf("task census: created list repeated %08X", at)
		}
		if len(seen) >= 200 {
			return 0, fmt.Errorf("task census: created list exceeds 200 tasks")
		}
		seen[at] = true
		off := at - memory.DRAMBase
		if r.RAM.Read(off+0x0c, bus.Word) != 0x5441534b {
			return 0, fmt.Errorf("task census: created list points to non-task at %08X", at)
		}
