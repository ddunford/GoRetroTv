...
func (r *Runtime) taskCount() (int, error) {
	count := 0
	err := r.walkTasks(func(_ string, _ uint8, _ uint32) { count++ })
	return count, err
}
...
func (r *Runtime) TaskState(name string) (status uint8, runs uint32, found bool, err error) {
	err = r.walkTasks(func(taskName string, taskStatus uint8, taskRuns uint32) {
		if taskName == name {
			status, runs, found = taskStatus, taskRuns, true
		}
	})
	return
}

func (r *Runtime) walkTasks(visit func(name string, status uint8, runs uint32)) error {
	var seed uint32
	for off := uint32(0); off+0x38 <= r.RAM.Size(); off += 4 {
		if r.RAM.Read(off+0x0c, bus.Word) != 0x5441534b {
			continue
		}
		if taskName(r.RAM, off) == "SMTTask" {
	...
	for at := seed; ; {
		if at < memory.DRAMBase || at-memory.DRAMBase+0x70 > r.RAM.Size() || at&3 != 0 {
		...
		visit(taskName(r.RAM, off), uint8(r.RAM.Read(off+0x18, bus.Byte)), r.RAM.Read(off+0x1c, bus.Word)) // #nosec G115 -- byte read.
		next := r.RAM.Read(off+4, bus.Word)
		if next == seed {
			return nil
		}
		at = next
...
func taskName(ram *memory.RAM, off uint32) string {
	var name [8]byte
	for i := range name {
		name[i] = byte(ram.Read(off+0x10+uint32(i), bus.Byte)) // #nosec G115 -- byte read
		if name[i] == 0 {
