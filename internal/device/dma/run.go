...
func (d *Controller) Run(channel uint8) error {
	...
	// The oracle records other channel starts and raises completion without moving data.
	// In particular, its ROM channel 5 has no transport destination behind it.
	switch channel {
	case 12:
		if d.blitter == nil {
