...
func TestWhatTheRowsSecondScalarMeans(t *testing.T) {
	...
	for i := range listings.Services {
		listings.Services[i].RowAt10 = assign[i]
		t.Logf("%-14s channel %4d  0xB2 second scalar (+10) = %d", listings.Services[i].Name,
	...
	settled := openAllChannelsFinished(t, press, ".artifacts/rowat10-grid.png")
	...
	if err := dumpScreen(t, box, "rowat10-grid.png"); err != nil {
		t.Fatal(err)
	}
	if len(states) == 0 {
		t.Fatalf("harness: no row type was stored at %#08X while the grid drew, so this says "+
			"nothing about service_type. Read .artifacts/rowat10-grid.png", uint32(stateWriteAt))
	}
	...
	t.Logf("READ .artifacts/rowat10-grid.png against .artifacts/rowstate-grid.png -- they are the " +
