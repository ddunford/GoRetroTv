...
- [x] `TASK-3c.7` ⫘ Tests → `/go-engineer` [TC-3c.1, TC-3c.2, TC-3c.3, TC-3c.4, TC-3c.5]
- [x] `TASK-3c.8` ⫘ Security audit → `/security-reviewer` [no-test: audit produces its own report]
...
- **Acking everything changes the boot.** The later oracle measurement reaches 42 tasks with
  either policy, but its display descriptor uses 8 bpp rather than 4 bpp under all-ack. The
  earlier 19-task stall was withdrawn; the default still answers only the measured commands.
