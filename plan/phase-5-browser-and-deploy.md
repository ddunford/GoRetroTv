...
- [ ] `TASK-5.9` The wire contract: one versioned schema for the framebuffer and key messages, generated or shared rather than written twice, with the TS decoder's fixture **captured from the running server** — a hand-written fixture matches the author's mental model, not the wire → `/go-engineer` [TC-5.7]
