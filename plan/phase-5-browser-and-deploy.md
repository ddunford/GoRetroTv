...
## Outcome
A visitor opens `<configured-public-host>`, waits for the box to boot, presses **sky**, and navigates the
...
- [x] `TASK-5.5` Production Dockerfile, compose overlay, Traefik labels and TLS at `<configured-public-host>` → `/devops-deployment-engineer` [TC-5.5]
