...
| Medium | `Dockerfile` uses mutable `golang:1.22` and `gcr.io/distroless/static-debian12:nonroot` tags. A rebuild could pick up different builder or runtime bytes without a source change. | Release task `gort-4sx.16` pins both images by digest after the supported Go toolchain is available. |
| Low | None | — |
...
- **Data and dependency integrity (A02, A06, A08):** Firmware is gitignored, excluded from the runtime image, and mounted read-only by Compose. The loader checks each image against the manifest in the operator-supplied firmware directory; this proves internal consistency, not authenticity of the selected firmware set. The project declares no external Go modules. `./ctl.sh vuln` passed against the reviewed baseline: 32 accepted findings, zero new. The old Go toolchain and mutable base-image tags remain release work.
...
Verification: `./ctl.sh test`, `./ctl.sh lint`, `./ctl.sh gate`, `./ctl.sh conformance`, and `./ctl.sh vuln` passed. The local startup gate verified all three firmware images, `/health`, build version, and graceful stop. The full guest-driven bootloader handoff acceptance is tracked as Phase 3c `gort-f3f.10` / TC-3c.7, after the CPU reaches the unmodelled video RAM port at instruction 3,204,424; see `docs/reference/cpu-oracle-measurement.md`.
