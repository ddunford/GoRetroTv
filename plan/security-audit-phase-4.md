...
- A real `firmwaretrace` process restored the private post-acquisition snapshot and listened on `127.0.0.1:23491`. A TCP connection to the host's LAN address `192.168.1.12:23491` was refused, while `127.0.0.1:23491` connected. The process exited normally after the local client disconnected. The existing real-client acceptance test also rejects `-gdb-addr 0.0.0.0:23457` and exercises register, memory, breakpoint and watchpoint operations.
...
- `https://goretrotv.demosrv.uk/health` returned HTTP 404 during this audit. The public URL is not yet serving this application, so the Phase 5 audit must verify the actual public route and its developer-surface isolation after deployment.
