...
def build(docs: Path, out: Path, title: str) -> int:
    ...
    order = sorted(groups, key=lambda g: (g != ".", g))
...
def source_hash(docs: Path) -> str:
    digest = hashlib.sha256()
    for path in sorted(docs.rglob("*.md")):
        digest.update(path.relative_to(docs).as_posix().encode() + b"\x00" +
...
def check(docs: Path, out: Path) -> int:
    if not out.exists():
...
def serve(docs: Path, out: Path, title: str, port: int, hosts: list[str]) -> int:
    ...
    def ensure() -> None:
        if page_hash(out) != source_hash(docs):
            build(docs, out, title)
    ...
    if a.cmd == "serve":
        ...
        if a.lan:
            ...
            if lan is None:
                ...
                return 1
            hosts.append(lan)
        return serve(docs, out, a.title, a.port, hosts)
