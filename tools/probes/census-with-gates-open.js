// WITH BOTH GATES OPEN, WHAT DOES THE KEY PRESS ACTUALLY CALL -- and is a glyph among it?
//
// The box now paints its own menu: eleven fills, five background bands and six 480x32 rows. Every
// one of them carries the same value, and the blitter registers say that is not an artefact of our
// decode -- across all eleven the ONLY words that vary are word 12 (destination x,y) and word 13
// (width,height). Nothing that could be a colour differs. The application really is filling eleven
// rectangles with one value.
//
// AND EVERY BLIT IS A FILL. Not one copy. Text on this hardware is a copy -- glyphs blitted from a
// source surface -- so "no text" and "no copies" are the same observation, and the question is
// which natives the press reaches and whether any of them is the one that would draw a glyph.
//
// The census counts at the shims, where every thunk has a distinct address, so the histogram is
// exact. Its key is built the page's way and a missing key THROWS, because a census that cannot
// find its own subject reported (1,0xE4) and (1,0xC7) as never called for two whole runs.
//
// THE BASELINE IS THE POINT. Module-1 activity here is bursty and periodic, so a count from a
// key-press window means nothing without the same count from a window with no key in it. Both are
// taken and only the difference is reported.
...
function snap(){
  var hits = window.__pcHits.apply(null, NAT.map(function(r){ return r[1]; })), o = {};
...
function diff(a, b){
  ...
  NAT.forEach(function(r){
    var d = b[r[0]] - a[r[0]];
    if (d > 0) out.push({ n: '(1,0x'+r[0].toString(16).toUpperCase().padStart(2,'0')+')', calls: d, fn: h(r[2]) });
  });
...
// BASELINE: an untouched box, same length as the press window.
var q0 = snap();
...
await new Promise(function(r){ setTimeout(r, 20000); });
var q2 = snap();
var newBlits = window.__blitLog().slice(blitsBefore);
var kinds = {};
newBlits.forEach(function(e){
  var k = (e.note || '').split(' ')[0] || '?'; kinds[k] = (kinds[k]||0)+1; });

return {
  idle20s: diff(q0, q1),
  press:   diff(q1, q2),
  blits: { n: newBlits.length, kinds: kinds,
           anyCopy: newBlits.some(function(e){ return (e.note||'').indexOf('copy') === 0; }) },
