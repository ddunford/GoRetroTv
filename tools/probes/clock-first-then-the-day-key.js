// sky-02me.21 -- GIVE THE BOX A CLOCK FIRST, THEN LET IT SAY WHICH DAY IT WANTS.
//
// WHERE THIS STARTS. twelve-records-and-the-guide.js got the whole of FUN_800c95d0's chain to turn:
// 20 sections, 240 records registered (exactly 12 per section, the number we wrote), the 0xB5 walk
// and the per-event register both at 240, registerBlock, dayKeyToDate, dateToBaseTime, and BOTH
// notifications. The store is full and the guide is byte-identical: 0xB4E3CAD8, 26 colours, the
// same "Further schedule information is not available" as an empty box.
//
// SO THE REMAINING GATE IS ABOUT *WHEN*, AND THE DECOMPILATION SAYS SO IN ONE LINE. For each record
// the parser does `local_30 = dateToBaseTime(dayKeyToDate(section[8..9])) + start * 2`. The two
// bytes at data[8..9] -- 0x9E8B, the ones the hardware unit filters on -- are fed to a DATE
// conversion, and every programme we send is placed relative to whatever day that yields. A guide
// showing "now" has nothing to draw if "now" is not inside that day.
//
// AND THE EARLIER REFUTATION DOES NOT COVER THIS. "0x9E8B is not the MJD" was measured twice, and
// both times by moving the clock AFTER the line-up and watching the bytes fail to move. That tests
// whether the box RE-SUBSCRIBES, not what the value means -- and the box programs its match unit
// once. This run inverts the order: the clock is set BEFORE anything asks for listings, and then
// whatever the unit demands is READ OFF __siMatches() and stamped, exactly as the writer was
// designed for. Whatever the bytes turn out to be, the section carries the ones the box named.
//
// AND THE FIRST RUN OF THIS PROBE ALREADY MOVED SOMETHING. With the clock set first the box
// subscribes to table **0xA0**, where every previous run saw 0xA1 -- and the parser reads
// `local_a6 = tableId & 3` and feeds it straight into `dayKeyToSlot(key, local_a6)`. So the low bits
// of the table id are part of WHICH DAY the section is filed under, they moved when the clock moved,
// and every run so far has been sending 0xA1 because that is what an unclocked box asked for. The
// table id is therefore read off the match unit here too, exactly like the two filter bytes: the
// hardware mask is 0xFE so both ids arrive either way, which is precisely why sending the wrong one
// is a silent mis-filing rather than a refusal.
//
// THE CONTROL IS THE PREVIOUS RUN, and it is a real one rather than a gesture: same probe, same
// payload, same presses, no clock. Its guide hash is 0xB4E3CAD8 at 26 colours, and it is asserted
// here as the baseline rather than quoted, so a difference is this run's doing.
