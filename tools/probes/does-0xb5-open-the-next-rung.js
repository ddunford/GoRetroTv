// sky-02me.20 -- FEED A WELL-FORMED 0xB5 AND SEE WHAT THE BOX ASKS FOR NEXT.
//
// THE LADDER IS THE INSTRUMENT, and it is the only one that has ever told this project where the
// data lives. A bare box subscribes to 0x40 and 0x73. One NIT opens 0x4A. A BAT carrying a line-up
// opens 0xA1/ext=0xBBB -- and 0xBBB was not a number anyone chose, it was the value fed in 0xB1
// entry field +3..4. Every rung has been found by feeding the previous one and reading
// __siMatches() afterwards.
//
// WHERE THAT LEAVES 0xA1. Its descriptor loop is exhausted as a home for the programmes: all 256
// tags swept, exactly one consumed (0xB5, two big-endian u16 out of desc[2..3] and desc[4..5], then
// return 1 which STOPS the walk), 0xBC walked-and-not-consumed, and 0xA0 measured as the same table
// as 0xA1 rather than a second one. The other handlers in the subsystem -- 0xB2, 0xB4, 0xBE -- are
// small bitfield extractors. None of them is a schedule.
//
// SO 0xB5 IS PROBABLY A REFERENCE, AND A REFERENCE POINTS SOMEWHERE. This feeds it distinctive
// values and watches the SUBSCRIPTIONS, not the memory: if either u16 turns up as a new table-id
// extension, or a new PID gets armed, that is the next rung and it names itself the way 0xBBB did.
//
// THE VALUES ARE CHOSEN SO THAT ANY NUMBER SEEN LATER NAMES THE FIELD IT CAME FROM -- one decade
// each, never equal, never zero, and never a value already in play (0x0BB8..0x0BBB are the
// listings ids, 0x1000 the bouquet, 0x0020 the network).
//
// AND A NEGATIVE HERE IS WORTH RECORDING RATHER THAN RETRYING. If a well-formed 0xB5 changes no
// subscription at all, then the reference is resolved INTERNALLY -- against the carousel the box
// already believes it has -- and the next subject is the carousel transport itself, not SI.
//
...
function subs(){
  return { tables: window.__siMatches().map(function(x){
             return '0x' + (x.tableId === null ? '??' : x.tableId.toString(16))
                  + (x.extension !== null ? '/ext=0x' + x.extension.toString(16) : ''); }).sort(),
           pids: window.__siFilters().filter(function(f){ return f.armed; })
                   .map(function(f){ return f.pid; }).sort() };
}
function diff(a, b){
  return { newTables: b.tables.filter(function(t){ return a.tables.indexOf(t) < 0; }),
           goneTables: a.tables.filter(function(t){ return b.tables.indexOf(t) < 0; }),
           newPids:   b.pids.filter(function(p){ return a.pids.indexOf(p) < 0; }) };
}

var HEAD = [0x9E, 0x8B, 0x42, 0x0B, 0x44, 0x45];
var VER = 0;
function buildA1WithB5(f1, f2){
  VER = (VER + 1) & 0x1F;
  // 0xB5 first: it stops the walk on a match, so anything behind it would never be reached --
  // proved in this project's own run, 35 reads against 981.
  var desc = [0xB5, 4].concat(u16(f1), u16(f2));
  var p = HEAD.concat(desc);
  var len = 5 + p.length + 4;
  var s = [0xA1, 0xB0 | ((len >>> 8) & 0x0F), len & 0xFF]
    .concat(u16(0x0BBB), [0xC1 | ((VER & 0x1F) << 1), 0x00, 0x00], p);
  var c = crc32(s);
  return s.concat([(c >>> 24) & 0xFF, (c >>> 16) & 0xFF, (c >>> 8) & 0xFF, c & 0xFF]);
}

// One decade each, distinct, non-zero, and not colliding with anything already in play.
var TRIALS = [
  { f1: 0x2345, f2: 0x6789, why: 'both fields distinctive, different decades' },
  { f1: 0x3456, f2: 0x789A, why: 'again, to tell a real mapping from a coincidence' },
  { f1: 0x0BB8, f2: 0x4567, why: 'f1 = a listings id the box already holds' },
  { f1: 0x4567, f2: 0x0BB8, why: 'the same value in the OTHER field, to say which one is read' }
];

for (var ti = 0; ti < TRIALS.length; ti++) {
  var t = TRIALS[ti];
  var before = subs();
  var r = { trial: hx(t.f1) + ' / ' + hx(t.f2), why: t.why, before: before };
  if (!wants(0xA1)) { r.error = 'the 0xA1 subscription lapsed -- a null result would be about timing'; out.passes.push(r); continue; }
  // Three copies with rising versions: one section cannot be told from a box that was busy.
  var ok = true;
  for (var k = 0; k < 3; k++) {
    var res = window.__siPush(A1_PID, buildA1WithB5(t.f1, t.f2));
    if (!res.ok) { r.error = 'push refused: ' + res.why; ok = false; break; }
    await new Promise(function(x){ setTimeout(x, 4000); });
  }
  if (!ok) { out.passes.push(r); continue; }
  await new Promise(function(x){ setTimeout(x, 8000); });
  r.after = subs();
  r.diff = diff(before, r.after);
  r.changed = r.diff.newTables.length > 0 || r.diff.newPids.length > 0;
  r.namesAField = r.diff.newTables.some(function(x){
    return x.indexOf(t.f1.toString(16)) >= 0 || x.indexOf(t.f2.toString(16)) >= 0; });
  out.passes.push(r);
}

out.anyChange = out.passes.some(function(p){ return p.changed; });
out.namedAField = out.passes.some(function(p){ return p.namesAField; });
out.tablesAtEnd = tablesWanted();
out.tasksAtEnd = window.__tasks().n;
out.headline = out.anyChange
  ? ('A WELL-FORMED 0xB5 MOVES THE SUBSCRIPTIONS'
     + (out.namedAField ? ' AND ONE OF ITS FIELDS NAMES THE NEW EXTENSION -- that is the next rung.'
                        : ' but neither field appears in the new subscription; read the diff.'))
  : 'A WELL-FORMED 0xB5 CHANGES NO SUBSCRIPTION. The reference is resolved internally, against the '
  + 'carousel the box already believes it has -- so the next subject is the carousel transport '
  + 'itself (modules, Huffman at 0x800BECF0, parsers near 0x800C95D0/0x800C9CA0) and NOT another '
  + 'SI table. Recorded as a negative rather than retried.';
return out;
