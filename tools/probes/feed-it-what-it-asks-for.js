// sky-02me.21 -- FEED IT WHATEVER IT ASKS FOR, ROUND AFTER ROUND, LIKE A REAL CAROUSEL.
//
// WHAT THE LINKAGE CHANGED. Giving the BAT a linkage_descriptor of type 0x91 flips the guide's one
// database question from the NOT-ANSWERED arm to the ANSWERED one -- proven A/B in a single run
// where the only difference between the two presses was seven bytes in a second BAT. It costs
// 14,944 extra o-code instructions and three extra events, so the guide does substantially more
// work with the answer than without it.
//
// AND THE BOX THEN ASKED FOR SOMETHING IT HAD NEVER ASKED FOR: a new PID channel, **0x30** -- the
// first of Sky's eight title PIDs -- appeared on the press taken with "now" inside the day we had
// fed. It already had 0x36 and 0x37. So the linkage unblocked acquisition and the box started
// walking days, and every previous probe fed ONE day on ONE PID under ONE table id, read once and
// then carried for the whole run.
//
// THAT IS THE THING TO STOP DOING. The table id, the listings PID and the two day bytes have all
// been observed MOVING -- with the clock, and now with the linkage -- and all three are read off
// __siMatches() and __siFilters(). A real carousel does not hand a box one day and stop; it repeats
// everything continuously and the box takes what it is currently asking for. So this re-reads the
// request EVERY ROUND and feeds a full twelve-programme day for each (table id, day key) on every
// listings PID the box has open, for ten rounds, recording every distinct key it was asked for.
//
// WHAT WOULD MAKE THIS READ FALSELY, and both are guarded: a round that feeds a key nobody asked
// for proves nothing, so the keys actually fed are reported rather than the keys intended; and a
// press taken while the box is rebuilding its channel list builds no widgets and its recovery
// afterwards reads exactly like listings arriving, so the baseline is asserted to have drawn.
//
// The BAT keeps the linkage, the clock is still set before anything asks for listings, and the
// records are still the twelve with the corrected length. Only the feed loop is new.
...
for (var rd = 0; rd < 10; rd++) {
  // WHAT IS THE BOX ASKING FOR *NOW*? Re-read all three every round. Each of them has been watched
  // moving during a run, and a loop that carries any one of them feeds a day nobody wants.
  var live = window.__siMatches().filter(function(x){ return x.tableId !== null && (x.tableId & 0xFC) === 0xA0; });
  ...
  pids.forEach(function(p){ if (out.pidsSeen.indexOf(hx(p)) < 0) out.pidsSeen.push(hx(p)); });
  if (!live.length || !pids.length) { await new Promise(function(r){ setTimeout(r, 2000); }); continue; }

  for (var li = 0; li < live.length; li++) {
    var unit = live[li];
    var lb = (unit.bytes || []).slice(6, 8).map(function(x){
      var mm = /^([0-9a-f]+)\/([0-9a-f]+)$/.exec(x); return mm ? parseInt(mm[1], 16) : null; });
    if (lb[0] === null || lb[1] === null) continue;
    TABLE_ID = unit.tableId;
    FILTER_BYTES = [lb[0], lb[1]];
    var key = hx(TABLE_ID) + '/mjd=' + ((lb[0] << 8) | lb[1]);
    if (out.keysFed.indexOf(key) < 0) out.keysFed.push(key);
    var lo = (unit.extension & unit.extensionMask) >>> 0;
    for (var ex = lo; ex <= unit.extension; ex++) {
      var sec = buildA1(ex);
      for (var pj = 0; pj < pids.length; pj++) {
        if (window.__siPush(pids[pj], sec).ok) pushed++; else refused++;
      }
    }
  }
...
out.listingsPidsFed = out.pidsSeen;
...
out.headline = 'THE ASK: ' + armOf(baseG) + ' / ' + armOf(fedG) + ' / ' + armOf(nowG) + '. '
