// sky-02me.21 -- IS THE "SIGNATURE 9E 8B" ACTUALLY THE MJD DATE THE BOX IS ASKING FOR?
//
// THE CLAIM UNDER TEST, and it withdraws one of this project's own recorded facts. The 0xA1 match
// unit reads:
//     bytes: ["a1/fe","b/ff","bb/fc","0/0","0/0","0/0","9e/ff","8b/ff","0/0","0/70"]
// and this file has called 9E 8B "the signature the hardware match unit demands" since it was
// found. Open-source readers of the same stream (jcdutton/loadepg, dave-p/openTVtoXML,
// vdr-plugin-eepg) say a Sky/OpenTV EPG TITLE section is:
//     Data[0]     table id, 0xA0..0xA4 and 0xB0 for titles (0xA8..0xAB, 0xB1 for summaries)
//     Data[3..4]  channel id            <- which is exactly our extension 0x0BBB
//     Data[8..9]  MJD DATE              <- which is exactly the "signature" bytes
//     Data[10..]  records: +0..1 event id, +2..3 length, +4 tag, +5.. Huffman title
//
// And 0x9E8B is 40587, which is the MJD of 1 JANUARY 1970 -- the epoch a box with no clock would
// hold. Checked against the anchor this project already uses, 1995-10-10 = MJD 50000.
//
// It also explains the ONE measurement that was never explained: fed a payload of self-naming
// bytes, the box read offsets 2,3,4,5,6,7 -- event id, length, tag, first data byte -- which is
// this record layout exactly, not a descriptor loop that happened to start at 6.
//
// THE TEST IS A PREDICTION, NOT AN INSPECTION. If those bytes are the date, then setting the box's
// clock must MOVE them: a TDT for 1998-01-01 should make the unit demand C6 7E (MJD 50814) and one
// for 1998-01-15 should make it demand C6 8C (50828). If they do not move, the date reading is
// wrong and "signature" stands.
//
// THE CONTROL IS THE UNIT ITSELF. Its table id, extension and masks must NOT change when only the
// clock does -- if the whole unit is reprogrammed differently, the two readings are not comparable.
//
...
function a1Unit(){
  var u = window.__siMatches().filter(function(x){ return x.tableId === 0xA1 || x.tableId === 0xA0; })[0];
  if (!u) return null;
  return { tableId: hx(u.tableId), tableIdMask: hx(u.tableIdMask),
           extension: hx(u.extension), extensionMask: hx(u.extensionMask),
           bytes: u.bytes, dateBytes: (u.bytes && u.bytes.length > 7) ? [u.bytes[6], u.bytes[7]] : null };
}
function mjdOf(y, m, d){
  var t = Date.UTC(y, m - 1, d) - Date.UTC(1858, 10, 17);
  return Math.round(t / 86400000);
}
function expectBytes(mjd){
  return [((mjd >> 8) & 0xFF).toString(16) + '/ff', (mjd & 0xFF).toString(16) + '/ff'];
}

out.before = a1Unit();
if (!out.before) throw new Error('no 0xA0/0xA1 match unit after the line-up -- nothing to test');
out.beforeDateBytes = out.before.dateBytes;

var TRIALS = [
  { y: 1998, m: 1, d: 1,  why: 'the in-world era opening day' },
  { y: 1998, m: 1, d: 15, why: 'a second date, so a single match cannot be a coincidence' }
];
for (var i = 0; i < TRIALS.length; i++) {
  var t = TRIALS[i], mjd = mjdOf(t.y, t.m, t.d);
  var r = { setClockTo: t.y + '-' + t.m + '-' + t.d, why: t.why,
            mjd: mjd, mjdHex: hx(mjd), expected: expectBytes(mjd) };
  // Several copies with the clock advancing, because one TDT can be missed and a single reading
  // cannot tell "never" from "not yet".
  for (var k = 0; k < 4; k++) {
    var res = window.__siTDT(t.y, t.m, t.d, 12, k, 0);
    if (!res.ok) { r.error = 'TDT refused: ' + res.why; break; }
    await new Promise(function(x){ setTimeout(x, 4000); });
  }
  await new Promise(function(x){ setTimeout(x, 10000); });
  r.unit = a1Unit();
  r.dateBytes = r.unit && r.unit.dateBytes;
  r.matchedPrediction = !!(r.dateBytes && r.dateBytes[0] === r.expected[0] && r.dateBytes[1] === r.expected[1]);
  // THE CONTROL: only the clock changed, so everything else about the unit must be identical.
  r.unitOtherwiseUnchanged = !!(r.unit && out.before
      && r.unit.tableId === out.before.tableId && r.unit.tableIdMask === out.before.tableIdMask
      && r.unit.extensionMask === out.before.extensionMask);
  out.passes.push(r);
}

out.movedAtAll = out.passes.some(function(p){
  return p.dateBytes && out.beforeDateBytes && (p.dateBytes[0] !== out.beforeDateBytes[0] || p.dateBytes[1] !== out.beforeDateBytes[1]); });
out.bothPredicted = out.passes.every(function(p){ return p.matchedPrediction; });
out.controlsHeld = out.passes.every(function(p){ return p.unitOtherwiseUnchanged; });
out.tablesAtEnd = tablesWanted();
out.tasksAtEnd = window.__tasks().n;
out.headline = out.bothPredicted
  ? 'CONFIRMED: the two bytes are the MJD DATE, and the box asks for the day its clock says. '
    + '"The signature 9E 8B" is withdrawn -- it was MJD 40587, 1 Jan 1970, the epoch of a box with '
    + 'no clock. Feeding a day means setting the TDT and stamping the same MJD in Data[8..9].'
  : (out.movedAtAll
      ? 'the bytes MOVED with the clock but not to the predicted MJD -- read the table; the mapping '
        + 'is real but is not plain MJD'
      : 'the bytes did NOT move with the clock, so they are not the date and "signature" stands');
return out;
