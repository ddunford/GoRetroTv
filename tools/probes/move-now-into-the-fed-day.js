// sky-02me.21 -- FEED THE DAY THE BOX ASKS FOR, THEN MOVE "NOW" INTO IT.
//
// THE DAY BYTES ARE THE MJD AFTER ALL, AND THE TWO EARLIER REFUTATIONS WERE BOTH ABOUT SOMETHING
// ELSE. Set the clock BEFORE anything asks for listings and every part of the request moves at once:
//
//     no clock          table 0xA1   PID 0x33          data[8..9] = 0x9E 0x8B  = MJD 40587 = 1 Jan 1970
//     clock 1 Jan 1998  table 0xA0   PIDs 0x37, 0x36   data[8..9] = 0xC6 0x7F  = MJD 50815 = 2 Jan 1998
//
// and the firmware's own converter agrees: __call(dayKeyToDate, 0xC67F) returns a structure carrying
// 2 and 0x62 -- day 2, year 98. 0x9E8B was never a signature and never an identifier the box merely
// holds; it is the date an UNCLOCKED box thinks it is, which is the Unix epoch. The two earlier tests
// moved the clock AFTER the match unit had been programmed and correctly observed that the bytes did
// not follow, because the box does not re-subscribe; that is a fact about re-subscription, not about
// meaning.
//
// WHICH LEAVES ONE GAP, AND IT IS ARITHMETIC RATHER THAN FORMAT. The box asks for the day AFTER the
// one its clock is in, so the day it wants never contains "now", and a guide showing now has nothing
// to draw however full that day is. The previous run proved the filling works -- 20 sections, 240
// records, exactly twelve per section, registerBlock, both notifications -- and the guide stayed
// byte-identical because every programme in it was tomorrow.
//
// SO THIS RUN DOES BOTH HALVES. It feeds whatever key the box names, re-reading it each round rather
// than carrying it, and THEN moves the clock forward onto the day it fed, and presses the guide
// again. Three guide presses in all: before the listings, after the listings, and after the clock has
// moved into them. Each is screenshotted, because a hash says "different" and not "what".
//
// AND THE CLOCK MOVE IS ITS OWN CONTROL. If the guide changes at press three and not at press two,
// the listings were there all along and the missing piece was the date; if it changes at neither,
// the store is not what the guide reads and that is a different finding with the filling already
// proven. The two are distinguishable because the presses are separated.
...
out.headline = 'AFTER THE CLOCK MOVE the guide is '
