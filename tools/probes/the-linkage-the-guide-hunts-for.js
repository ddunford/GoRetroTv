// sky-02me.21 -- GIVE THE GUIDE THE LINKAGE IT IS LOOKING FOR.
//
// WHAT THE GUIDE ACTUALLY ASKS, read off the firmware and then counted on the running box. A tv
// guide press makes exactly FOUR module-12 calls -- (12,0x6) x2, (12,0x8), (12,0x23), (12,0x26) x2
// -- and module 12 is the EPG database: (12,0x23) resolves through the live module table to
// 0x800CB770, in the same neighbourhood as everything FUN_800c95d0 uses to FILL the store.
//
// 0x800CB770 is lock / ask / unlock around 0x800CB69C, which presets the caller's out-parameter to
// 0xFFFFFFFF and searches the SI the box holds for a **linkage_descriptor, tag 0x4A**, via
// 0x800AD608 with the callback at 0x800CB660. That callback is eight instructions of arithmetic and
...
//
// and the caller then requires the search to have returned 3 or 4 AND `out+9 == 0x91`. On success
// it calls 0x800A4040(ctx, service_id, tsid, onid), which walks the box's own 0x18-byte service
// records looking for that service on that transport. On failure it calls 0x800AC774(ctx) and the
// guide says "Further schedule information is not available".
//
// MEASURED, ON A REAL PRESS: the ask happens ONCE, with arguments (0x00000000, 0x80494CC4), and the
// **NOT-ANSWERED arm runs**. So the guide never reaches the event store at all -- which is exactly
// why its 37,610-instruction build is byte-identical on a box holding 240 of our programmes and on
// one holding none. Filling the store harder could never have moved it.
//
// SO THE ONE THING THIS CHANGES IS THE BAT: it now carries a linkage_descriptor of type 0x91 naming
// one of the four services the same BAT declares, on the transport the box itself reports through
// __siIds(). Everything else -- clock first, the day the box asks for, the twelve corrected records,
// the three presses -- is move-now-into-the-fed-day.js unaltered, so the comparison is line for
// line with the run that established the not-answered arm.
//
// IT GOES IN BOTH DESCRIPTOR LOOPS ON PURPOSE. Which structure 0x800AD608's handle names -- the
// bouquet-level loop or the per-transport one -- is not established, and two four-minute runs to
// find out before knowing whether the mechanism works at all is the wrong order. If the answered
// arm fires, the location is a bisect worth doing; if it does not, neither loop was it and the
// handle is somewhere else entirely.
//
// THE THREE PRESSES SEPARATE THE TWO CHANGES. Press one has the linkage and NO listings: if the
// answered arm runs there, the linkage is doing its job independently of anything we store. Press
// two adds the listings, press three moves "now" into the day they were filed under.
