...
var rp = 0x33;
// ---- SUMMARIES, the half of this format we have never sent ------------------------------------
// Titles parse but their TEXT is never read, so the text is fetched by something else. Summaries
// are tables 0xA8..0xAB / 0xB1; a record is [event id][packet_length] then descriptors, tag 0xB9
// holding Huffman text, and the reference reader attaches one to a title it ALREADY HOLDS keyed on
// (event id, mjd) -- hence titles first, same event ids.
// BOTH candidate PIDs are tried, because the line-up arms 0x33 AND 0x52 and a title pushed at 0x52
// read zero. __siMatches indexes match units while __siFilters indexes PID channels, so they must
// not be joined by index; pushing to each is the measurement that avoids the question.
var SUM_VER = 0;
function buildSummary(ext, tableId){
  SUM_VER = (SUM_VER + 1) & 0x1F;
  var payload = [(ext >> 8) & 0xFF, ext & 0xFF, 0xC1 | ((SUM_VER & 0x1F) << 1), 0x00, 0x00,
                 SIGNATURE[0], SIGNATURE[1]].concat(SUMMARIES);
  var len = payload.length + 4;
  var s = [tableId, 0xB0 | ((len >> 8) & 0x0F), len & 0xFF].concat(payload);
  var c = crc32(s);
  return s.concat([(c >>> 24) & 0xFF, (c >>> 16) & 0xFF, (c >>> 8) & 0xFF, c & 0xFF]);
}
async function summaryPass(name, pid, tableId, ext){
  var r = { pass: name, pid: hx(pid), tableId: hx(tableId), extension: hx(ext) };
  var SMARK = SUMMARIES.slice(6, 14);
  var p0 = window.__siPush(pid, buildSummary(ext, tableId));
  if (!p0.ok) { r.error = 'push refused: ' + p0.why; return r; }
  await new Promise(function(x){ setTimeout(x, 8000); });
  var ringPhys = (parseInt(p0.at, 16) >>> 0) & 0x1FFFFFF;
  var found = window.__find(SMARK, 0x80000000, 0x82000000, 24).filter(function(a){
    var q = (parseInt(a, 16) >>> 0) & 0x1FFFFFF; return !(q >= ringPhys && q < ringPhys + 2048); });
  r.copiesFound = found;
  if (!found.length) { r.answer = 'delivered and never copied out of the ring'; return r; }
  var chosen = found.filter(function(a){ return (parseInt(a, 16) >>> 0) >= 0x80200000; })[0] || found[0];
  r.watching = chosen;
  var mid = parseInt(chosen, 16) >>> 0;
  window.__readWatch((mid - 0x2000) >>> 0, (mid + 0x2000) >>> 0);
  var p1 = window.__siPush(pid, buildSummary(ext, tableId));
  if (!p1.ok) { window.__readWatch(); r.error = 'second push refused'; return r; }
  await new Promise(function(x){ setTimeout(x, 8000); });
  var log = window.__readWatchLog();
  window.__readWatch();
  r.watch = { reads: log.reads };
  var again = window.__find(SMARK, (mid - 0x2000) >>> 0, (mid + 0x2000) >>> 0, 8);
  if (!again.length) { r.error = 'the marker is not in the watched region afterwards'; return r; }
  var at = (parseInt(again[0], 16) >>> 0) - 6;
  var pcs = {}, offs = {};
  log.all.forEach(function(rr){
    var a = parseInt(rr.at, 16), n = rr.size || 1, k;
    for (k = 0; k < n; k++) { var o = (a + k) - at;
      if (o < 0 || o >= SUMMARIES.length) continue;
  ...
  r.reachedTheText = r.offsetsRead.some(function(o){ return o >= 6 && o < 40; });
  r.parsed = r.readers.length > 0;
  r.answer = r.parsed ? ('READ BY ' + r.readers.map(function(p){ return p.pc; }).join(', ')) : 'nothing read it';
  return r;
}
out.guide = {};
...
out.summaries = [];
for (var si = 0; si < EXTS.length; si++) {
  out.summaries.push(await summaryPass('0xA8 on PID 0x33, ext ' + hx(EXTS[si]), 0x33, 0xA8, EXTS[si]));
  out.summaries.push(await summaryPass('0xA8 on PID 0x52, ext ' + hx(EXTS[si]), 0x52, 0xA8, EXTS[si]));
  if (out.summaries.some(function(x){ return x.parsed; })) break;
}
out.summaryParsed = out.summaries.some(function(x){ return x.parsed; });
