// sky-02me.21 -- WHAT DID THE GUIDE ACTUALLY SUBSCRIBE FOR?
//
// THE GUIDE IS A SUBSCRIBER, NOT A READER, and that is measured rather than argued: all 51 of
// module 12's functions censused across three guide presses on a box holding 480 of our programmes
// give the same four calls as a bare box. It never queries for a row. What it does call is
// (12,0x26) = 0x800C6154, the NOTIFICATION COLLECTOR, which dispatches only when a 44-byte slot's
// +4 word holds 0x3EC or 0x3EA -- the exact two ids FUN_800c95d0 sends -- and then writes 999 back.
//
// AND 0x800C579C, THE NOTIFY WE ALREADY TRIGGER FORTY TIMES A FEED, IS THE OTHER HALF:
//
//     for table = 1..2:
//       header = 0x80165048 + table * 0x10      count at +0 (u16), slot array pointer at +8
//       for slot = 1 .. count-1:                slots are 0x2C bytes
//         slot+0x00 == 1                        active
//         slot+0x04 == the id being notified    it is waiting for this one
//         slot+0x1D != 2 ? slot+0x10 == service : slot+0x12 == service
//         slot+0x18 == dayKey  &&  slot+0x1A == tableId & 3
//         -- or slot+0x28 == 1, which matches regardless
//
// So a notification only reaches the guide if the guide is registered for that service, that day and
// that table-id pair. We feed all three read off __siMatches() and still measure ZERO novel o-code
// events in a feed window against an idle control of the same length. Nothing we send fires a slot.
//
// THIS DUMPS THE SLOTS RATHER THAN REASONING ABOUT THEM. Taken three times -- before the guide has
// ever been opened, immediately after the guide press, and after the listings -- so a slot the GUIDE
// registered is distinguishable from one that was always there, and a slot our feed CLEARED is
// distinguishable from one that never fired. The layout above is read off the two functions that
// use it, so every field printed has a reason to be printed.
//
// A HEADER THAT DOES NOT PARSE IS A HARNESS FAILURE, NOT AN EMPTY TABLE. A count of zero and a
// pointer into nothing look identical in a dump, and "the guide subscribed to nothing" is exactly
// the kind of conclusion this task has already had to withdraw once. Both are asserted.
...
out.headline = 'SLOTS active: ' + out.slots.beforeAnyGuide.activeSlots + ' before any guide, '
