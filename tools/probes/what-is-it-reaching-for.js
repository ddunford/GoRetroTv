// sky-02me.21 -- THE BOX SAYS "No satellite signal is being received". WHAT IS IT REACHING FOR?
//
// WHERE THIS STARTS. Giving the BAT a linkage_descriptor of type 0x91 made the guide's one database
// question succeed -- the ANSWERED arm (0x800A4040) now runs on every guide press, where the
// not-answered arm (0x800AC774) used to. The guide gained a widget and eight colours, and the
// screenshot says what it did with the answer:
//
//     FOR YOUR INFORMATION
//     No satellite signal is being received
//
// That string is at DS+0xC193, string id 0x51B66, and it sits in a family of service-availability
// messages -- "There is a technical fault with this channel", "Please try later". So the guide
// followed the linkage, tried to SELECT the service behind it, and something in the front-end path
// said no. This is the first gate the box has named itself.
//
// TWO EXPLANATIONS AND THEY WANT DIFFERENT WORK. Either 0x800A4040 failed to match the linkage's
// (service_id, tsid, onid) against the box's own service records and an unresolvable service reads
// as unavailable -- in which case the fix is in the BAT. Or it matched, the box genuinely tried to
// tune and acquire, and the layers below the section rings are what is missing -- no PAT, no PMT,
// nothing on the transport at all. We inject reassembled sections straight into the demux rings, so
// everything under that seam is unmodelled.
//
// THE MEASUREMENT SEPARATES THEM WITHOUT GUESSING, because a box that is TRYING TO TUNE says so in
// three places at once and a box that gave up earlier says nothing in any of them:
//
//   * __demod() -- the satellite demodulator at I2C 0x18. It is modelled as ALWAYS LOCKED, so a
//     tune attempt shows up as transactions rather than as a failure. Transactions during the press
//     mean the front end was driven.
//   * __siFilters() / __dispState().pids -- a box selecting a service asks for PID 0x0000 (PAT) and
//     then a PMT PID. Neither has ever been requested on this box. A new filter is the box asking
//     for something we have never had to send.
//   * __siMatches() -- a new table id (0x00 PAT, 0x02 PMT) is the same statement one layer up.
//
// A COUNT OF ZERO IN ALL THREE IS ITSELF THE ANSWER and points the work back at the BAT; anything
// new in any of them points it at the transport. Both are reported, neither is assumed.
//
// Everything else is the-linkage-the-guide-hunts-for.js unaltered, so the arms census and the
// screens are comparable press for press with the run that produced the message.
...
// The three places a box that is trying to tune says so. Read together, never summarised: "the
// front end was touched" and "a new PID was asked for" are different answers and collapsing them
// loses the one that decides what to build.
function frontEnd(){
  var d = window.__demod(), ds = window.__dispState();
  return { demodReads: d.reads, demodWrites: d.writes, demodLog: d.log.length,
           pids: (window.__siFilters() || []).map(function(f){ return f.pid; }).sort().join(' '),
           dispPids: JSON.stringify(ds.pids || null),
           tables: window.__siMatches().map(function(x){
             return (x.tableId === null ? '??' : hx(x.tableId))
                  + (x.extension !== null ? '/' + hx(x.extension) : ''); }).sort().join(' ') };
}
function frontEndDiff(a, b){
  return { demodReads: b.demodReads - a.demodReads, demodWrites: b.demodWrites - a.demodWrites,
           pidsBefore: a.pids, pidsAfter: b.pids, pidsChanged: a.pids !== b.pids,
           tablesBefore: a.tables, tablesAfter: b.tables, tablesChanged: a.tables !== b.tables,
           dispPidsChanged: a.dispPids !== b.dispPids, dispPidsAfter: b.dispPids };
}
...
async function press(raw, label, tag, from){
  ...
  var s0 = surface(), a0 = arms(), fe0 = frontEnd();
  window.__key(raw, 0);
  ...
  return { tag: tag, key: label, pressedFrom: from, widgets: c.newWidget || 0,
           ...
           frontEnd: frontEndDiff(fe0, frontEnd()) };
}
...
out.frontEndVerdict = (function(){
  var g = [baseG, fedG, nowG];
  var touched = g.some(function(p){ return p.frontEnd.demodReads || p.frontEnd.demodWrites; });
  var asked = g.some(function(p){ return p.frontEnd.pidsChanged || p.frontEnd.tablesChanged; });
  if (!touched && !asked)
    return 'THE BOX NEVER REACHED FOR THE FRONT END. No demodulator traffic and no new PID or table '
         + 'across any guide press, so "no satellite signal" is a conclusion it drew without asking '
         + 'the hardware -- the work is back in the BAT, not under the section rings.';
  return 'THE BOX DID REACH FOR THE FRONT END'
       + (touched ? ' (demodulator traffic)' : '')
       + (asked ? ' (new PID or table asked for)' : '')
       + ' -- the layers under the section rings are what is missing.';
})();
out.headline = out.frontEndVerdict + ' THE ASK: ' + armOf(baseG) + ' / ' + armOf(fedG) + ' / ' + armOf(nowG) + '. '
