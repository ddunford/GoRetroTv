// THE BOX IS ASKING FOR TABLE 0xA1 -- WHAT DOES IT DO WITH ONE? -- sky-02me.5 / sky-eluc.12.
//
...
var out = { question: 'what does this box do with a table 0xA1 section',
...
function payload(withSignature){
  var b = [];
  for (var k = 0; k < PAYLOAD_LEN; k++) b.push((0x40 + k) & 0xFF);
  // payload[3] is masked 0x70 and must be zero in those bits; 0x43 is not, so give it a value that
  // is distinctive and satisfies the unit.
  b[3] = 0x0B;
  if (withSignature) { b[0] = SIGNATURE[0]; b[1] = SIGNATURE[1]; }
  return b;
}
