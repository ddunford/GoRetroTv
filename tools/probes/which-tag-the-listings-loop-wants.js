// sky-02me.17 -- WHICH DESCRIPTOR TAG CARRIES THE PROGRAMMES? The 0xA1 sweep.
//
...
out.passes = [];
out.passes.push(await pass('A: 0x01..0x40', range(0x01, 0x40)));
out.passes.push(await pass('B: 0x41..0x80', range(0x41, 0x80)));
out.passes.push(await pass('C: 0x81..0xC0', range(0x81, 0xC0)));
out.passes.push(await pass('D: 0xC1..0xFF', range(0xC1, 0xFF)));
// Tag 0x00 on its own, with two descriptors AFTER it. If those are walked, 0x00 is an ordinary tag
// in this pointer-bounded loop; if they are not, 0x00 terminates it after all and the BAT sweep's
// one blind spot is this loop's too.
out.passes.push(await pass('E: tag 0x00, with 0x11 and 0x22 behind it to prove it is not a terminator',
                           [0x00, 0x11, 0x22]));
